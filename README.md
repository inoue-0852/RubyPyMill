...
## 🧪 動作確認（Dry Run）
```bash
bundle exec ruby bin/ruby_pymill   exec examples/notebooks/sample.ipynb   --output examples/outputs/out.ipynb   --params examples/params/demo.json   --kernel python3   --dry-run
```
...
## 💻 使い方（CLI）
```bash
ruby_pymill run <input_ipynb>   --output <output_ipynb>   [--kernel python3]   [--params params.json]   [--cwd path]   [--dry-run]
```
